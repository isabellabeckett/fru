...
```{r}
saveRDS(FW_combined_data, "/Users/bhumpanya/Documents/Greg Jefferis' lab/fru/RDSfiles/FW_combined.RDS")
```
...
```{r}
#CAN RUN, BUT HAS ALREADY BEEN SAVED AS RDS
...
```{r}
#UPDATE SPECIFIC NEURONS OR CLONES
#s <- length(to_do)
FW_neurons <- unlist(`FWsheet_FW_aDT-c`$root_id)
for(i in FW_neurons)
{
  message(paste("Processing neuron", i))
  FW_partners <- tryCatch({
    suppressWarnings({
      suppressMessages({
    choose_segmentation('public')
    flywire_partner_summary(i, partners = "inputs", threshold = 2)
      })
    })
  }, error = function(e) {
    message(paste("Error for neuron", i, "- Skipping neuron"))
  ...
  if (!is.null(FW_partners)) {
    saveRDS(FW_partners, paste0("~/Downloads/FW_partners2/",i, ".RDS"))
  }
}
```
...
saveRDS(FW_clone_N_in, "/Users/bhumpanya/Downloads/FW_clone_N_in2.RDS")
...
FW_clone_N_in <- readRDS("/Users/bhumpanya/Downloads/FW_clone_N_in2.RDS")
...
ggplot(data = FW_complete_hemilineages2, aes(x = clone, y = percent_dimorphic_input, color = Fru, shape = Fru)) +
  ...
  labs(
    x = "Clone",
    y = "Percent Fru+ input",
    title = "Percent Fru+ input by clone and Fru expression",
    color = "Fru",
    shape = "Fru"
  ) +
  theme_minimal() +
...
ggplot(data = FW_complete_hemilineages2, aes(x = clone, y = percent_dimorphic_output, color = Fru, shape = Fru)) +
  ...
  labs(
    ...
    title = "Percent Fru+ output by clone and Fru expression",
    color = "Fru",
    shape = "Fru"
  ) +
  theme_minimal() +
