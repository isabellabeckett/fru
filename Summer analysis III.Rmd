...
#Plot comparing Observed and Expected
...
```{r}
#Plot to show how complete identification has been
MCNS_found <- sum(summary$`Found?` == "Y")
MCNS_entire_lineage <- sum(summary$`Entire lineage checked?` == "Y", na.rm = TRUE)
FW_found <- sum(summary$`Found in FW?` == "Y")
FW_entire_lineage <- sum(summary$`Manual FW?` == "Y")
  level <- c("MCNS completed?", "MCNS Found?", "FW completed?", "FW Found?") 
  count <- c(MCNS_entire_lineage, MCNS_found, FW_entire_lineage, FW_found) 
  no_count <- c(58-MCNS_entire_lineage, 58-MCNS_found, 58-FW_entire_lineage, 58-FW_found)
...
df_long <- completeness %>%
  ...
  mutate(level = factor(level, levels = c("MCNS completed?", "MCNS Found?", "FW completed?", "FW Found?"))) %>%
...
```{r}
#Plot showing difference of hemilineage sizes between MCNS and FW
merged_df_all_info <- googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1RvzMYSPB01eG1X_sI6Xrp48s4XgwdXjTrNoGCtR39BI/edit?gid=753368775#gid=753368775", sheet = "Fru_FW")
