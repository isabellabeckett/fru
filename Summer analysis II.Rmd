...
for (i in seq_along(sheetnames)) {
  ...
  # Use tryCatch to handle errors if a sheet doesn't exist
  tryCatch({
    ...
    # Assign the modified data to a new variable in the global environment
...
saveRDS(all.Y, "/Users/bhumpanya/Downloads/allY.RDS")
saveRDS(combined_data, "/Users/bhumpanya/Downloads/combined.RDS")
```
...
ggplot(complete_clones, aes(x = average_dimorphic_input, y = average_dimorphic_output, color = clone)) +
  ...
  labs(x = "Percent Dimorphic Input", y = "Percent Dimorphic Output", title = "Male Fru+ Input vs Output") +
  theme_minimal() +
...
complete_hemilineages2 <- complete_hemilineages2 %>%
  arrange(Fru)
ggplot(data = complete_hemilineages2, aes(x = clone, y = percent_dimorphic_output, color = Fru, shape = Fru)) +
  geom_jitter(width = 0.2, size = 3, stroke = 1.2, fill = NA) +  # Plot the bodyid values with color and shape
  geom_point(data = mean_values_o, aes(x = clone, y = mean_percent_dimorphic_output, shape = Fru), size = 5, color = "black",stroke = 1.5, show.legend = FALSE) +  # Plot the mean values
  scale_shape_manual(values = c("Y" = 1, "N" = 0)) + 
   scale_color_manual(values = c("Y" = "orange2", "N" = "darkorchid2"))+# Dot (16) for Fru == "Y", Square (15) for Fru == "N"
  labs(
    x = "Clone",
...
partner58449 <- merge(out58449, combined_data[, c("bodyid", "clone", "hemilineage/AOTU00")], 
                      by = "bodyid", 
                      all.x = TRUE)
```
...
```{r}
complete_hemilineages2 %>% complete_hemilineages2[bodyid != "",]
```
...
```{r}
# Subset the data for Fru = Y and Fru = N
data_fru_Y <- subset(FW_complete_hemilineages2, Fru == "Y")
data_fru_N <- subset(FW_complete_hemilineages2, Fru == "N")
