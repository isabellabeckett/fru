...
for(cl in clone)
{
  ...
  subset <- FW_combined_data[FW_combined_data$clone == cl & FW_combined_data$Fru =="N", "root_id"]
  subset <- unlist(subset)
  s <- length(subset)
  for(i in 1:s)
  {
...
```{r}
png(file= "~/Documents/Greg Jefferis' lab/fru/network_female_i_nonfrutofru_scheme.png",
width=3000, height=3000)
