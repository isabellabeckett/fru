...
summary <- googlesheets4::read_sheet(fru.url, sheet = "Summary")
class <- summary %>% select(Clones, Putative_Class)
...
```{r}
png(file= "~/Documents/Greg Jefferis' lab/fru/network_female_o_frutofru_scheme.png",
...
scaled_edge_widths <- edge_weights*20 / max(edge_weights) 

# Set edge color with transparency (e.g., "black" with 50% transparency)
transparent_edge_color <- adjustcolor("black", alpha.f = 0.5)

colnames <- tibble(colnames(FW_adj))
colnames(colnames) <- "Clones"
node_categories <- left_join(colnames, class, by = "Clones")
node_categories <- unlist(node_categories[,2])
...
```{r}
png(file= "~/Documents/Greg Jefferis' lab/fru/network_female_i_frutofru_scheme.png",
width=3000, height=3000)
...
# Scale the edge weights to determine the edge widths
scaled_edge_widths <- edge_weights*20 / max(edge_weights)   # Adjust the multiplier for more differentiation

# Set edge color with transparency (e.g., "black" with 50% transparency)
transparent_edge_color <- adjustcolor("black", alpha.f = 0.5)

colnames <- tibble(colnames(FW_adj2))
colnames(colnames) <- "Clones"
node_categories <- left_join(colnames, class, by = "Clones")
node_categories <- unlist(node_categories[,2])
...
# Initialize an empty vector to store edge colors
edge_colours <- rep(NA, length(E(network2)))

# Iterate over each edge and assign color based on direction
for (i in seq_along(E(network2))) {
  edge <- E(network2)[i]
  source_node <- ends(network2, edge)[1]  # Get the source node name
  target_node <- ends(network2, edge)[2]  # Get the target node name
...
clone <- clone[clone != "pSP-e"]
clone <- clone[clone != "pIP-j"]
clone <- clone[clone != "pSG-e"]
clone <- clone[clone != "aSG-a"]
clone <- clone[clone != "pMP-g"]
c = length(clone)
MCNS_adj <- matrix(nrow = c, ncol = c)
colnames(MCNS_adj) = rownames(MCNS_adj) = clone
for(ca in clone)
{
  for(cb in clone)
  {
    ...
    if(length(percent_by_pair) > 0 && !is.na(percent_by_pair))
    {
    MCNS_adj[canum, cbnum] <- percent_by_pair
    }else{
    MCNS_adj[canum, cbnum] <- 0
    }
  }
}
```
...
```{r}
png(file= "~/Documents/Greg Jefferis' lab/fru/network_male_o_frutofru_scheme.png",
width=3000, height=3000)

network3 <- graph_from_adjacency_matrix(MCNS_adj, mode = "directed", weighted = TRUE, diag = FALSE)

edge_weights <- E(network3)$weight

scaled_edge_widths <- edge_weights*20 / max(edge_weights) 

colnames <- tibble(colnames(MCNS_adj))
colnames(colnames) <- "Clones"
node_categories <- left_join(colnames, class, by = "Clones")
node_categories <- unlist(node_categories[,2])
...
edge_colours <- rep(NA, length(E(network3)))

for (i in seq_along(E(network3))) {
  edge <- E(network3)[i]
  source_node <- ends(network3, edge)[1]  
  target_node <- ends(network3, edge)[2]  
...
#clone <- clone[clone != "pMP-g"]
c = length(clone)
MCNS_adj2 <- matrix(nrow = c, ncol = c)
colnames(MCNS_adj2) = rownames(MCNS_adj2) = clone
for(ca in clone)
{
  for(cb in clone)
  {
    filtered_allclone_i_com <- allclone_i_com %>% filter(query_clone == ca)
    percent_by_pair <- filtered_allclone_i_com %>% filter(clone == cb) 
    percent_by_pair <- unlist(percent_by_pair$percent_weight)
    canum <- which(clone == ca)
    cbnum <- which(clone == cb)
    if(length(percent_by_pair) > 0 && !is.na(percent_by_pair))
    {
    MCNS_adj2[canum, cbnum] <- percent_by_pair
    }else{
      MCNS_adj2[canum, cbnum] <- 0
    }
  }
}
```
...
```{r}
png(file= "~/Documents/Greg Jefferis' lab/fru/network_male_i_frutofru.png",
width=3000, height=3000)

network4 <- graph_from_adjacency_matrix(MCNS_adj2, mode = "directed", weighted = TRUE, diag = FALSE)

edge_weights <- E(network4)$weight

scaled_edge_widths <- edge_weights*20 / max(edge_weights) 

colnames <- tibble(colnames(MCNS_adj2))
colnames(colnames) <- "Clones"
node_categories <- left_join(colnames, class, by = "Clones")
node_categories <- unlist(node_categories[,2])
...
edge_colours <- rep(NA, length(E(network4)))

for (i in seq_along(E(network4))) {
  edge <- E(network4)[i]
  source_node <- ends(network4, edge)[1]  
  target_node <- ends(network4, edge)[2]  
