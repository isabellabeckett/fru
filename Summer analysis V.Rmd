...
for(i in 1:s)
{
  ...
  FW_temp_clone <- merge(partners, FW_combined_data[, c("root_id", "clone")], 
                      by = "root_id", 
                      all.x = TRUE)
  FWpartner_aDT_b <- rbind(FWpartner_aDT_b, FW_temp_clone)
}
```

```{r}
FW_aDT_b_tot <- sum(FWpartner_aDT_b$weight)
FWpartner2_aDT_b <- FWpartner_aDT_b %>% filter(!is.na(clone))
FW_aDT_b_partner_weight <- FWpartner2_aDT_b %>%
  group_by(clone) %>%
  summarize(total_weight = sum(weight, na.rm = TRUE))
FW_aDT_b_partner_weight$percent_weight <- 100*FW_aDT_b_partner_weight$total_weight/FW_aDT_b_tot
FW_aDT_b_partner_weight <- FW_aDT_b_partner_weight %>% arrange(desc(percent_weight))
FW_aDT_b_partner_weight$clone <- factor(FW_aDT_b_partner_weight$clone, 
                                         levels = FW_aDT_b_partner_weight$clone)
```

```{r}
ggplot(FW_aDT_b_partner_weight, aes(x = clone, y = percent_weight, fill = clone)) +
...
```{r}
partner_aDT_c <- data.frame()
clone_neurons <- unlist(combined_data[combined_data$clone == "aSP-i" & combined_data$Fru == "Y", "bodyid"])
  ...
  clone_conn <- merge(connection_table, complete_hemilineages[, c("bodyid", "clone", "Fru")], 
                      by = "bodyid", 
                      all.x = TRUE)
  partner_aDT_c <- rbind(partner_aDT_c, clone_conn)
```

```{r}
aDT_c_tot <- sum(partner_aDT_c$weight)
partner2_aDT_c <- partner_aDT_c %>% filter(!is.na(clone))
aDT_c_partner_weight <- partner2_aDT_c %>%
  group_by(clone, Fru) %>%
  summarize(total_weight = sum(weight, na.rm = TRUE))
aDT_c_partner_weight$percent_weight <- 100*aDT_c_partner_weight$total_weight/aDT_c_tot
aDT_c_partner_weight <- aDT_c_partner_weight %>%
  filter(Fru == "Y") %>%
  arrange(desc(percent_weight))
aDT_c_partner_weight$clone <- factor(aDT_c_partner_weight$clone, 
                                         levels = aDT_c_partner_weight$clone)
```

```{r}
ggplot(aDT_c_partner_weight, aes(x = clone, y = percent_weight, fill = clone)) +
  geom_bar(stat = "identity") + 
  labs(x = "Clone", y = "Percent Weight", title = "Percent Weight by Clone") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
